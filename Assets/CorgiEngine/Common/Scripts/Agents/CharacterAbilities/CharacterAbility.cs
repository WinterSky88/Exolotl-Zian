...
{	
	...
	public class CharacterAbility : MonoBehaviour 
	{
        ...
        //Added by Efra
        [Tooltip("Added by Efra. An array containing all the blocking weapon states. If the Character's weapon is in one of these states and tries to trigger this ability, it won't be permitted. Useful to prevent this ability from being used while a weapon is in use, for example.")]
        public Weapon.WeaponStates[] BlockingWeaponStates;
        protected CharacterHandleWeapon[] WeaponList;
        //Added by Efra
                        ...
                        for (int i = 0; i < BlockingWeaponStates.Length; i++) {
                            for (int x = 0; x < WeaponList.Length; x++) {
                                if (WeaponList[x].CurrentWeapon != null) {
                                    if (BlockingWeaponStates[i] == (WeaponList[x].CurrentWeapon.WeaponState.CurrentState)) {
                                        return false;
                                    }
                                }
                            }
		...
		{
            //Added by Efra
            WeaponList = this.gameObject.GetComponents<CharacterHandleWeapon>();
            //Added by Efra
